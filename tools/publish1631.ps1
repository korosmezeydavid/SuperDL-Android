...
$src = Join-Path $dir 'SuperDL-1.63.1-release.apk'
if (-not (Test-Path $src)) { throw "Nincs meg a kiadasi APK: $src" }
Get-ChildItem $dir -Filter 'SuperDL-1.63.0*.apk' | Remove-Item -Force -ErrorAction SilentlyContinue
Copy-Item $src (Join-Path $dir 'SuperDL.apk') -Force
Copy-Item $src (Join-Path $dir 'SuperDL-1.63.1.apk') -Force
