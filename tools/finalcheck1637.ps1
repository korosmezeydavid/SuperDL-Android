...
Write-Output '--- 3. SMS SZEREPKOR: mind a negy kotelezo osszetevo ---'
if ($aapt) {
    $tree = & $aapt.FullName dump xmltree --file AndroidManifest.xml $apk 2>$null
...
Write-Output '--- 4. KEPERNYOOLVASO: a ket kisegito szolgaltatas ---'
if ($aapt) {
    $tree = & $aapt.FullName dump xmltree --file AndroidManifest.xml $apk 2>$null
    foreach ($p in @('ScreenReaderService', 'WAKE_LOCK')) {
        $van = $tree | Select-String -Pattern $p
        Write-Output ("    " + $p.PadRight(26) + $(if ($van) { "MEGVAN" } else { "HIANYZIK" }))
    }
}

Write-Output '--- 5. hangok a KIADASI APK-ban ---'
