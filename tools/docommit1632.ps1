$ErrorActionPreference = 'Stop'
Set-Location 'C:\Users\msn\Documents\SuperDL-Android'
git add app/build.gradle
git add app/src/main/kotlin
git add app/src/main/assets
git add tools/relnotes1632.md tools/reltitle1632.txt tools/verzio1632.json tools/commitmsg1632.txt
git add tools/publish1632.ps1 tools/build_release1632.ps1
git add tools/atirat2.py tools/atirat2.txt
git add dokumentumok
Write-Output '--- staged ---'
git diff --cached --name-only
Write-Output '--- titok-szken (URESNEK kell lennie) ---'
git diff --cached --name-only | Select-String -Pattern 'keystore|ably_key|local.properties'
Write-Output '--- commit ---'
git commit -F tools/commitmsg1632.txt
git push origin master
