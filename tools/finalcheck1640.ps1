...
if ($aapt) {
    $tree = & $aapt.FullName dump xmltree --file AndroidManifest.xml $apk 2>$null
    foreach ($p in @('SMS_DELIVER', 'WAP_PUSH_DELIVER', 'RESPOND_VIA_MESSAGE', 'BROADCAST_WAP_PUSH', 'SEND_RESPOND_VIA_MESSAGE', 'ScreenReaderService', 'WAKE_LOCK', 'FOREGROUND_SERVICE_DATA_SYNC', 'CatalogDownloadService', 'SmsSendReceiver', 'BootReceiver', 'SCHEDULE_EXACT_ALARM')) {
        $van = $tree | Select-String -Pattern $p
        Write-Output ("    " + $p.PadRight(30) + $(if ($van) { "MEGVAN" } else { "HIANYZIK" }))
    }
}

Write-Output '--- 3b. AZ S.O.S. BEALLITO RECEIVER VEDELME ---'
# MIERT ITT: az MK-V M4-hez (otthon-figyeles) ez blokkolo elofeltetel volt.
# A felmeres tevesen "vedtelennek" mondta; a manifestben ott a
# protectionLevel="signature". Ez az ellenorzes attol ored, hogy egy kesobbi
# atrendezes csendben le ne vegye.
if ($aapt) {
    $tree = & $aapt.FullName dump xmltree --file AndroidManifest.xml $apk 2>$null
...
Write-Output '--- 9. MK-V M0 + M1 (ez a kiadas lenyege) ---'
