...
gh release view v1.63.2 --repo korosmezeydavid/SuperDL-Android --json assets |
    ConvertFrom-Json | ForEach-Object { $_.assets } |
    ForEach-Object { Write-Output ($_.name + "  " + $_.size + " byte") }

Write-Output '--- 4. a zarkepernyo-hangok benne vannak-e a KIADASI APK-ban ---'
...
Write-Output ("$($k.Count) klip, osszesen $([math]::Round(($k | Measure-Object -Property Length -Sum).Sum/1024)) kB")
