...
if ($aapt) {
    $tree = & $aapt.FullName dump xmltree --file AndroidManifest.xml $apk 2>$null
    foreach ($p in @('SMS_DELIVER', 'WAP_PUSH_DELIVER', 'RESPOND_VIA_MESSAGE', 'BROADCAST_WAP_PUSH', 'SEND_RESPOND_VIA_MESSAGE', 'ScreenReaderService', 'WAKE_LOCK', 'FOREGROUND_SERVICE_DATA_SYNC', 'CatalogDownloadService', 'SmsSendReceiver', 'ScheduledSmsReceiver', 'BootReceiver', 'SCHEDULE_EXACT_ALARM')) {
...
Write-Output '--- 10. MK-V M2 + M3 (ez a kiadas lenyege) ---'
